...
10. **Cleanup leftovers:** `scripts/cleanup_stale_test_threads.py` (pushed, on box after pull) cancels #7041 + #6861 and rejects [TEST] #6190/#6244 — **Anjana must run it** (classifier blocks): `cd /home/lexi/AI_Scheduling_Agent && git pull --ff-only && set -a && . ./.env && set +a && .venv/bin/python scripts/cleanup_stale_test_threads.py`. Still manual afterward: sweep `[TEST]` emails + Lexi drafts from mailboxes. (#6235/#6481/#7181 already rejected.)
