...
**ARCHITECTURE UPGRADE (2026-08-04, `2e60c43`, deployed) — LLM-first request understanding.** Decision after the C-1/C-2 pattern (all accuracy defects were regex-parse gaps, zero calendar-math defects): keep the deterministic engine+gate, replace the brittle ears. The delegation path ran `use_llm_plan=False` (regex only), and even when on, the LLM could only return a label re-parsed by the same regex. Now Hermes returns **explicit `window_start`/`window_end` dates** (resolved from a `today` field the payload previously lacked) plus `earliest_hour`/`latest_hour`, all validated and clamped by code (past→rejected/clamped, >45-day spans discarded, ungrounded single-day refused, 7:00 floor per V-1); regex remains base + fallback; engine/gate untouched. **Verified live:** "the week after Labor Day" → Sep 7–13 with zero regex changes; "late afternoon" → 15:00 floor enforced in the candidate trace; the C-2 phrase resolves identically on the LLM path. Nuance logged: for "early morning… the earlier the better" the model leaves `latest_hour` null (no hard cap), where the regex capped at 11:00 — defensible, watch draft tone when mornings are full. Groups C-3+ run on this path.

**C-3 / I-1 / I-2 — ✅ PASS after the biggest defect chain of the run** (proposal 6475, lunch ask → delegation → escalation → guidance → single-slot offer). The lunch rule held perfectly (no lunch offered unprompted). Everything after it needed fixing:
- **I-1 escalation misdiagnosed the block** (`15e0f2d`): said "zero candidates… want me to move Patrick/WOB?" when Kory's own exception-only rule was the cause. Packet now carries a computed `policy_block_reason`; composer leads with it; "move Kory's meetings" demoted to last-resort-question, never for DO-NOT-MOVE/training/coach. Verified offline — corrected text names the rule and offers real options.
- **Hermes routing**: told "retry scheduling for proposal 6475", it improvised a "fully booked" answer from an ad-hoc calendar read instead of calling `lexi_retry_scheduling` (which would have errored anyway — see next). Tool docstring rewritten with trigger phrasings + "the engine is the authority" (`e91fce8`). Its conflict-check refusal to book over Kory's named noon times was verified CORRECT against the calendar — blocker 3 behaving well.
- **The guidance chain was severed in SEVEN independent places**, each a drifted copy of the same two rules (lunch exception-only, 2-slot minimum): (1) retry tool rejected status `needs_kory` that escalation writes (`e91fce8`); (2) guidance never reached validator preferences — `load_scheduling_preferences(guidance=)` added, `_LUNCH_YES` regex taught "lunch approved" word order (`17460cf`); (3) candidate menu had NO noon times — lunch menu 11:30/12:00/12:30/13:00 added (`a56cc77`); (4) ladder + (5) escalation gate + (6) pre-approval gate + (7) card-time offer_refresh each enforced 2-slot/default-prefs independently — all now guidance-aware; **Kory-directed searches may offer a single slot** (`a56cc77`, `18f17fb`, `530945e`, `bf659be`, `476b7a6`).
- **End state verified in prod DB:** 6475 `pending_approval`, slot **Fri Aug 21 11:30–12:30 MT** (genuinely free), correct Lexi draft. ~40 new tests; suite 520.
- **Architecture lesson recorded:** the seven-copies pattern is the same "duplicated helpers drift" failure as the name-formatting bug — consolidating rule enforcement onto one preferences+minimum authority is post-sign-off refactor #1.

**C-4 — ✅ PASS after one fix** (proposal 6481, "Tuesday afternoon or Wednesday next week"). The LLM parser resolved the window perfectly (Aug 11–12); slots were real, afternoon, Tue/Wed-patterned — but a week late with **no disclosure**: `maybe_shift_plan_window` (travel shift, V-3-correct) replaces the sender's window BEFORE the engine runs, so the gate honestly saw slots "in window" and no warning fired. Fixed `0c59feb`: the shift now sets `window_expanded` with the pre-shift label, so gate warning + persisted note both name it. Verified live: note reads *"no availability for Tuesday afternoon or Wednesday next week — offering week of August 17 (after travel) instead."*

**C-5 — ✅ PASS after one build** ("how about sometime Thursday instead?" on the unsent 6481 offer). The follow-up deduped correctly (no second proposal — B-4 regression holds) but only pinged Kory, leaving the stale Tue/Wed draft one approve-tap from ignoring her reply. Built `89d8a6f`: a follow-up on a **pending_approval** (unsent) offer folds the sender's text into the thread body and reruns the scheduler (sent offers keep notify — Group H owns those; sender text deliberately never routes through `kory_scheduling_guidance`, which can unlock policy exceptions). Verified live: regenerated draft acknowledges the reply, includes **Thu Aug 20 2:00** plus Tue/Wed options, no fabricated conflict, travel-shift note attached.

**C-6 — ✅ PASS, GROUP C COMPLETE 6/6** (proposal 6487, "coffee near Cherry Creek, next few weeks"). Coffee stayed coffee; the draft volunteered real Cherry Creek venues (Olive & Finch, Aviano); 9:30 starts on Wednesdays (training-day in-person floor); 60-min offer with 90-min reserve; two slots across two weeks (Aug 26, Sep 2) — the OB-4 scarcity presented honestly with the ⚠️ note. **Cosmetic defect logged (polish list): the shifted-window label names only its first week ("week of August 18") while the multi-week window runs through Sep 2** — correct dates, misleading phrasing (`_shift_plan_window` label convention).
...
### RUN 5 — Group O (outbound drafting stack), 2026-08-05 — ✅ COMPLETE

**O-1 ✅** (Denver trip): clarify-first on an empty topic, then correct preview via the stateless `lexi_draft_outbound_email` flow (by design no #id for non-scheduling drafts); audit confirmed zero sends. **O-2a ✅ with fix `a287cfe`**: draft content right, but channel inference required the full "Let's Win" block — bare "Kory" sign-offs (what chat drafts have pre-finalizer) routed to the LEXI mailbox: identity mismatch + slips the kory-channel block. Inference now reads the actual sign-off line; whichever name signs last wins.
**O-2b ✅ after the day's biggest chain** (#6820 final): (1) docstrings route availability-offering emails to `lexi_start_scheduling` (`8b16e77`) — hand-picked times from a calendar read have no holds/validation/acceptance-tracking; (2) that tool's cold path ran a 40-80s calendar fetch INSIDE an open SQLite write transaction (starving all writers — likely the real T0-2 culprit) and the 5-min cache TTL made every command pay it: slot search now precedes the transaction, TTL 30 min, Lexi's calendar writes invalidate the cache (`b199f7b`); (3) `TypeError: naive vs aware datetimes` killed EVERY lexi_start_scheduling run — private conflict-check copy replaced by shared busy_intervals (`9b41d75`); (4) after `/new` cleared chat context: hallucinated "Kory Kogon" subject, hard-coded kory voice on an "as Lexi" ask, run-on bullets — voice_mode now flows tool→proposal columns (voice decides mailbox), docstring pins Kory = Kory Mitchell, drafted_reply fields get Teams break normalization (`79f5b2f`); (5) cold outbound drafts opened "Thanks for reaching out" to someone who never wrote — `build_scheduling_reply(outbound=True)` (`f6ddd1b`). End-to-end CLI verification: 34.5s, staged, engine slots identical to the hand-picked ones.
**O-4 ✅ with fix `0c2f51a`**: threaded onto LT-C4, correctly reused the REJECTED #6481 via the retry path, guided single slot (Thu Aug 20 2:00) — but yesterday's travel-shift note survived the clean redo (`COALESCE` kept stale notes; scheduler_agent copy of the C-2 lesson). Also good caution: refused to blindly confirm "Thursday" when Aug 6/13 were booked, asked first. **O-5 ✅** (the critical one): explicit `send that email now` on a Kory-voice draft → honest refusal, nothing sent, audit clean. **O-6 ✅**: revision persisted through the update tool, sign-off intact (emoji-on-"warmer" noted for the Phase 4 style rulings). **O-7 ✅**: To/CC split correct, clarify-first, Kory voice.

### RUN 5 — Groups G + K, Phase 2 flip — 2026-08-05 ✅ PHASE 1 COMPLETE

**G-2 ✅ with 3 fixes** (#6827): ET-first from the fake NY signature worked and the profile row learned `America/New_York (signature)`. Defects fixed + deployed (`1cd3260`): "grab 30 minutes / quick sync" triaged as **coffee** (60-min in-person + Cherry Creek venues "to meet in NY") — triage prompt now requires an explicit coffee/drinks/dinner ask, virtual is the default; compose must describe offered dates truthfully (body claimed "next week" while offering Aug 26–Sep 8) and never attach Denver venues to non-Denver meetings.
**G-3 ✅ with 1 fix** (#6829, `00b0ecd`): first run came out MT-only — the prior-thread scan interleaved per-thread, so the newest thread's Gmail **Date header (-0600, Anjana's physical location)** answered before the older thread's NY signature was scanned. Scan is now two passes (all text evidence first, headers last). Redo verified live: all three slots ET-first with correct MT conversions. Learned profile row deleted post-test (prior-thread signatures remain — by design, re-derivable). Also fixed en route: a box-side merge conflict on `data/kory_voice_profile.json` (trivial `built_at_epoch`, resolved keeping newer; file was root-owned — chowned to lexi).
**K-1 ✅**: `remember: no meetings before 9 AM on Tuesdays` → semantic key `no_meetings_before_9am_tuesdays`; the 8:30 correction **updated the same row** (updated_at stamped, no duplicate). **K-2 ✅**: service restart → fact persisted AND the fresh process recalled it correctly in chat. The 8:30-Tuesday fact is left in place (remove at Phase 4 cleanup if unwanted).
**PHASE 2 FLIP ✅ 2026-08-05 ~03:40 UTC** (Anjana ran it; `.env.bak.phase2-flip.*` saved): `LEXI_KORY_OUTBOUND_BLOCKED=false`; runtime posture re-verified — REQUIRE_KORY_APPROVAL=true, AUTO_EXECUTE=false, ALLOW_IMMEDIATE_SEND=false. From here `approve #N` sends real email on both channels.

**HEIDI ESCALATION REMOVED (`622fc4e`, deployed)** — per Anjana (prompted by O-5's refusal offering to "flag this for Heidi"): Kory is the ONLY escalation target. `heidi_escalation.py` → `kory_escalation.py` (Teams-to-Kory only), MCP tool renamed `lexi_escalate_to_kory` ("never offer to flag/forward/hand off to anyone else"), briefing/email/forward machinery + `HEIDI_*`/`LEXI_HEIDI_ESCALATION_ENABLED` env vars deleted everywhere incl. box `.env` (0 legacy `needs_heidi` rows). Heidi-as-real-person data handling (calendar alias, seeds, signature parsing) kept. Suite 544.

### RUN 6 — P3-A `LT-D1`: the full real lifecycle — 2026-08-05 ✅ COMPLETE (sends OPEN)

One thread (#6835, anjanakummetha@gmail.com → kory@, CC lexi@) exercised the entire machine live: staged draft → edit-by-text → `approve` → **first real email send** → **first real calendar holds (3)** → real-inbox logo/signature render check (Gmail: good) → acceptance parse → **first real invite** (Teams online meeting, attendee correct) → unused holds released → **reschedule** (new offer → accept Wed Aug 19 11 ET → new invite + original Aug 10 meeting auto-deleted + Outlook cancellation to attendee) → **cancel** (`cancel meeting #6835 — recipient request` → event deleted, proposal `cancelled`, calendar left with zero Anjana/HOLD events). D-1/D-2/D-5/E-1/E-2/E-7/E-8/H-1/H-2/H-7/O-3-live all pass **after 10 defects found & fixed this run**:

1. **`b919dae` counter stall killed the first send**: `record_composio_call` (runs before every real Composio call) inherited the 30s busy_timeout; inside the approval transaction every Outlook step stalled 30s → gateway 120s timeout → its keepalive reconnect killed the worker AFTER the CID attach but BEFORE `OUTLOOK_SEND_DRAFT` (orphan draft deleted by hand). Counter now uses its own 250ms connection. Teams had advised "approve again" — double-send risk; docstring now forbids that advice after timeouts.
2. **`4f78295` hold placement not resumable**: 2nd approve sent the email but the hold step died on a DB lock after creating slot 1's calendar event and before its row — orphan event; the retry then (a) treated ANY existing holds as a complete set (caller early-return) and (b) saw its own orphan as a "conflict". Per-slot resume + own-orphan adoption (same `HOLD:` title + exact interval ⇒ adopt event id).
3. **`3d006a7` pending_invite invisible to the text path**: the invite-approval Teams push went missing and the accepted proposal was stranded — `pending` listed only pending_approval, typed `approve #N` dead-ended. `pending` now shows "Invites ready to send"; `approve #N`/`reject #N` handle pending_invite.
4. **`e8473c8` replies on booked meetings were dropped in silence** (the audit predicted this class): `_handle_generic_lexi_followup`'s notify list excluded `executed`. Now: reschedule wording → regenerate offer (existing invite untouched until new time confirmed); cancel wording → Kory ping, calendar untouched; else → generic ping. New `proposals.invite_event_id` column; invite dispatch deletes the prior invite (move semantics) and records the new id.
5. **`e0b6b99` hold placement isolated + Hermes truth guard**: both live hold runs died on the same lock race (approval connection vs orchestrator/webhook writers) — holds now run on their own connection, 3 attempts, failure audited un-rollback-ably. Hermes also told Kory an invite was "already sent and confirmed" while the proposal sat pending_invite — docstring: natural-language send confirmations route as `approve #N`; no send may be claimed without a same-turn tool result.
6. **`d496622` `cancel meeting #N` command**: the cancel ping asked Kory to say the word but no command existed (no calendar-delete MCP tool; reject refuses executed). New typed command deletes the tracked invite (attendee gets Outlook notice), records decision `cancelled`.
7. **`58f2c12` cancel/reschedule wording outranks the inbound-time detector**: "cancel our Wednesday call" got hijacked into an `inbound_time_blocked` ping because it mentions a weekday.
8. **`b642dab` 'cancel' added to the router's MANDATORY list**: Hermes answered `cancel meeting #6835` from its own reasoning ("cancel it manually") because the docstring list lacked the prefix.
